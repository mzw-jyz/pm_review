...
outcomes <- list(
  diabetes = list(type = "binary",
                  label = "Diabetes",
                  aliases = c("diabetes_cata2", "diabetes", "Diabetes", "T2D", "t2d", "DM2", "dm2", "type2dm", "type2_diabetes")),
  hypertension = list(type = "binary",
                      label = "Hypertension",
                      aliases = c("hypertension_if", "hypertension", "Hypertension", "HTN", "htn", "high_bp", "highbp", "sbp_htn")),
  obesity = list(type = "binary",
                 label = "Obesity",
                 aliases = c("obesity_if", "obesity", "Obesity", "OB", "ob"))
)
...
coerce_binary01 <- function(x, nm) {
  x0 <- safe_numeric(x)
  ux <- sort(unique(x0[!is.na(x0)]))
  ok <- all(ux %in% c(0, 1))
  if (!ok) {
    stop(
      paste0("Outcome column ", nm, " must be binary coded as 0/1. Observed values: ",
             paste(head(ux, 20), collapse = ", ")),
      call. = FALSE
    )
  }
  x0
...
qv <- function(x) paste0("`", gsub("`", "", as.character(x)), "`")
qv_vec <- function(x) vapply(x, qv, character(1))
...
first_stage_stats <- function(df, exposure_col, score_col, covars) {
  form0 <- as.formula(paste(qv(exposure_col), "~", paste(qv_vec(covars), collapse = " + ")))
  form1 <- as.formula(paste(qv(exposure_col), "~", paste(qv_vec(c(score_col, covars)), collapse = " + ")))
  m0 <- lm(form0, data = df)
  m1 <- lm(form1, data = df)
  a  <- anova(m0, m1)
  rss0 <- deviance(m0)
...
run_2sri_binary <- function(df, outcome_col, exposure_col, score_col, covars) {
  ...
  form2 <- as.formula(paste(qv(outcome_col), "~ ._xhat + ._res1 +", paste(qv_vec(covars), collapse = " + ")))
...
run_2sls <- function(df, outcome_col, exposure_col, score_col, covars, binary_outcome = FALSE) {
  rhs1 <- paste(qv_vec(c(exposure_col, covars)), collapse = " + ")
  rhs2 <- paste(qv_vec(c(score_col, covars)), collapse = " + ")
  form <- as.formula(paste(qv(outcome_col), "~", rhs1, "|", rhs2))
...
run_observational <- function(df, outcome_col, exposure_col, covars, type) {
  form <- as.formula(paste(qv(outcome_col), "~", paste(qv_vec(c(exposure_col, covars)), collapse = " + ")))
  if (type == "binary") {
...
egger_estimate <- function(beta_exp, se_exp, beta_out, se_out) {
  w <- 1 / (se_out^2)
  tmp <- data.frame(beta_out = beta_out, beta_exp = beta_exp, w = w)
  fit <- lm(beta_out ~ beta_exp, weights = w, data = tmp)
...
build_crossfit_score <- function(df, snp_cols, exposure_col, covars, K = 5) {
  ...
  for (k in seq_len(K)) {
    ...
    names(b) <- snp_cols
    for (s in snp_cols) {
      fm <- lm(as.formula(paste(qv(exposure_col), "~", paste(qv_vec(c(s, covars)), collapse = " + "))), data = dtr)
      est <- extract_coef(fm, s)
      b[s] <- est$beta
    }
    betamat[k, ] <- b
    cf_score[idx_te] <- as.numeric(as.matrix(dte[, snp_cols, with = FALSE]) %*% b)
  }
...
read_pheno <- function() {
  ...
  for (nm in names(outcome_map)) {
    col <- outcome_map[[nm]]
    if (outcomes[[nm]]$type == "binary") {
      ph[[col]] <- coerce_binary01(ph[[col]], col)
    } else {
...
make_same_sample_summary <- function(df, snps, exposure_col, outcome_col, outcome_type, covars) {
  ...
  for (i in seq_along(snps)) {
    s <- snps[i]
    fm_exp <- lm(as.formula(paste(qv(exposure_col), "~", paste(qv_vec(c(s, covars)), collapse = " + "))), data = df)
    e <- extract_coef(fm_exp, s)

    if (outcome_type == "binary") {
      fm_out <- glm(as.formula(paste(qv(outcome_col), "~", paste(qv_vec(c(s, covars)), collapse = " + "))), data = df, family = binomial())
    } else {
      fm_out <- lm(as.formula(paste(qv(outcome_col), "~", paste(qv_vec(c(s, covars)), collapse = " + "))), data = df)
    }
...
run_one_combo <- function(ph, outcome_map, exposure, threshold, outcome_nm) {
  ...
  dat[, GRS_weighted := as.numeric(as.matrix(dat[, snp_cols, with = FALSE]) %*% w)]
  dat[, GRS_unweighted := rowSums(.SD), .SDcols = snp_cols]
  ...
  if (outcome_type == "binary") {
    ...
    res_main <- rbindlist(list(
      cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                       threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "weighted_GRS"), main_res),
      cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                       threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "weighted_GRS"), lpm$result),
      cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                       threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "NA"), obs)
    ), fill = TRUE)
    ...
    res_main <- rbindlist(list(
      cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                       threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "weighted_GRS"), tsls$result),
      cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                       threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "NA"), obs)
    ), fill = TRUE)
  ...
  smry <- make_same_sample_summary(dat, snp_cols, exposure, outcome_col, outcome_type, covars)
  ...
  cf <- build_crossfit_score(dat, snp_cols, exposure, covars, K = 5)
  ...
  if (outcome_type == "binary") {
    ...
    res_cf <- rbindlist(list(
      cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                       threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "crossfit_weighted_GRS"), cf_res),
      cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                       threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "crossfit_weighted_GRS"), cf_lpm$result)
    ), fill = TRUE)
    ...
    res_cf <- cbind(data.table(exposure = exposure, outcome = outcome_nm, outcome_label = outcome_lab,
                               threshold = threshold, n = nrow(dat), n_iv = length(snp_cols), score = "crossfit_weighted_GRS"), cf_tsls$result)
